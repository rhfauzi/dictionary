...
import { changeStatusBusinessUnit } from 'src/api/business-unit'
...
export default function SectionUpdateStatus() {
  ...
  const handleChangeStatus = async () => {
    ...
    runProcess('Wait for updating business unit')
    stopProcess()
    await changeStatusBusinessUnit(dataForm)
      .then((res) => {
        if (res?.status === 200) {
  ...
  return (
    ...
    >
      ...
      <Typography.Title level={5} style={{ margin: 0 }}>
        {`Are you sure want to ${dataForm?.is_active ? 'activate' : 'inactivate'} ${dataForm?.code ?? 'this Business Unit'} ?`}
      </Typography.Title>
