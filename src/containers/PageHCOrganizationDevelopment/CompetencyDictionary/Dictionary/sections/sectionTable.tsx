...
import { getListCompetency } from 'src/api/competency-dictionary/dictionary'
  ...
  useEffect(() => {
    ...
    if (datas?.data?.length <= 0) {
      setPayloadEmpty({
        ...
        subtitle: 'Try another like Dictionary Name',
      })
    }
  }, [filters?.search])
  ...
  return (
    <>
      <Table
        ...
        columns={columns(handler) as any}
        loading={loading}
        ...
        // }}
        pagination={false}
        locale={{ emptyText: <EmptyState {...payloadEmpty} /> }}
      />
        ...
        <FloatAction>
          <Row justify="space-between" style={{ flexGrow: 1 }}>
            <b style={{ lineHeight: '48px' }}>
              {selectedRowKeys?.length} Competency name are selected
            </b>
            <Row gutter={10}>
              {/* <Col>
                <Button
