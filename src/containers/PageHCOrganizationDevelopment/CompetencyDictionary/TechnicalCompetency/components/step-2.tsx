...
import {
  ...
  getSubJobFamily,
  getJobTitle,
} from 'src/api/competency-dictionary/technical-competency'
...
export default function Step2({
  ...
  const handleGetJobFamily = async (index: string) => {
    try {
      await getJobFamily('')
        .then((res: any) => {
          ...
          if (type === 'details') {
            const getId = res.find(({ key }) => key === index)
  ...
  useEffect(() => {
    ...
    handleGetLegalEntiy()
    handleGetJobTitle()
    handleGetJobFamily(payload?.job_family_code)
  }, [])
            ...
            <Row gutter={[20, 20]}>
              ...
              <Col xs={12} xl={12}>
                <DebounceSelectSearch
                  ...
                  fetchOptions={(search) => getSubJobFamily(search)}
