...
import { useKPIResultContext } from '../../states/useContext'
import { createKPISetting } from 'src/api/kpi-result'
...
export default function ConfirmSubmit() {
  ...
  const func = showModal === 'create' && createKPISetting
  ...
  return (
    <Popup>
      ...
      <Typography.Title level={5} style={{ margin: 0, fontWeight: 'bold' }}>
        {isModuleCreate
          ? 'Are you sure want to save KPI Result ?'
          : 'Are you sure want to update KPI Result ?'}
      </Typography.Title>
      <div style={{ display: 'flex', gap: 10 }}>
        ...
        <Button
          ...
          className="hc-button"
          onClick={async () => {
            runProcess(`Wait for ${isModuleCreate ? 'submitting' : 'updating'} KPI Result`)
