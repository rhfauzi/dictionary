...
import { performanceMgtMenu } from './MenuData'

const PagePerformanceManagement = () => {
  const router = useRouter()

  return (
    <div style={{ height: '85vh' }}>
      <Container style={{ display: 'flex', flexDirection: 'column' }}>
        <Text variant="headingLarge">Performance Management</Text>
        <div style={{ fontSize: '11px', color: '#888888' }}>
          View your KPI Setting, Result, Monitoring and Performance Rating.
        </div>
      </Container>

      <Spacer size={50} />
      <Row gutter={[20, 20]} align="middle">
        {performanceMgtMenu.map((item: any, index: number) => (
          <Col
            key={index}
            xs={12}
            sm={12}
...
export default PagePerformanceManagement
