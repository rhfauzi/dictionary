...
import { updateCountry } from 'src/api/country'
...
export default function SectionUpdateStatus() {
  ...
  const handleChangeStatus = async () => {
    ...
    runProcess('Wait for updating country')
    stopProcess()
    await updateCountry(dataForm)
      .then((res) => {
        if (res?.status === 200) {
  ...
  return (
    ...
    >
      ...
      <Typography.Title level={5} style={{ margin: 0 }}>
        {`Are you sure want to ${dataForm?.is_active ? 'activate' : 'inactivate'} ${`${dataForm?.code} - ${dataForm?.name}` ?? 'this country'} ?`}
      </Typography.Title>
