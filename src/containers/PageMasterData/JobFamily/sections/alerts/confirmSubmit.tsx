...
import {
  updateJobFamily,
  createJobFamily,
} from 'src/api/job-family'
import { useJobFamilyContext } from '../../states/useContext'
...
export default function ConfirmSubmit() {
  ...
  const func = showModal === 'create' ? createJobFamily : updateJobFamily
  ...
  return (
    <Popup>
      ...
      <Typography.Title level={5} style={{ margin: 0, fontWeight: 'bold' }}>
        {isModuleCreate
          ? 'Are you sure want to save Job Family ?'
          : 'Are you sure want to update Job Family ?'
        }
      </Typography.Title>
      <div style={{ display: 'flex', gap: 10 }}>
        ...
        <Button
          ...
          className='hc-button'
          onClick={async () => {
            runProcess(`Wait for ${isModuleCreate ? 'submitting' : 'updating'} Job Family`)
