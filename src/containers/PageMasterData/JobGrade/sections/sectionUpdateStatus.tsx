...
import { changeStatusJobGrade } from 'src/api/job-grade'
...
export default function SectionUpdateStatus() {
  ...
  const handleChangeStatus = async () => {
    ...
    runProcess('Wait for updating Job Grade')
    stopProcess()
    await changeStatusJobGrade(dataForm)
      .then((res) => {
        if (res?.status === 200) {
  ...
  return (
    ...
    >
      ...
      <Typography.Title level={5} style={{ margin: 0 }}>
        {`Are you sure want to ${dataForm?.is_active ? 'activate' : 'inactivate'} ${`${dataForm?.grade} - ${dataForm?.name}` ?? 'this Job Grade'} ?`}
      </Typography.Title>
