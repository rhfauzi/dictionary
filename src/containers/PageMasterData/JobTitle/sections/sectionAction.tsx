...
import { downloadTemplate, downloadData, uploadTemplate } from 'src/api/job-title'
...
export default function SectionAction() {
  ...
  async function downloadDataAction() {
    try {
      ...
      link.href = url
      link.setAttribute('download', 'job-title.xlsx')
  ...
  async function downloadTemplateAction() {
    try {
      ...
      const link = document.createElement('a')
      link.href = url
      link.setAttribute('download', 'template-job-title.xlsx')
      document.body.appendChild(link)
      link.click()
      document.body.removeChild(link)
    } catch (error) {
          ...
          <Col md={16} sm={24} xs={24}>
            <Row align="middle">
              ...
              <Input
                required
                type="select"
                placeholder="Search Job Title Name, Description"
                value={search}
                onChange={(e: any) => {
