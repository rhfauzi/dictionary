  ...
  useEffect(() => {
    ...
    if (datas?.data?.length <= 0) {
      setPayloadEmpty({
        ...
        subtitle: 'Try another Job Title Name.',
      })
    }
  }, [filters?.search])
      ...
      {selectedRowKeys?.length > 0 && (
        <FloatAction>
          <Row justify="space-between" style={{ flexGrow: 1 }}>
            <b style={{ lineHeight: '48px' }}>{selectedRowKeys?.length} Job Family are Selected</b>
            <Row gutter={10}>
              <Col>
                <Button
                  size="big"
