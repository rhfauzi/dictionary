...
import { changeStatusDepartmentAndDivision } from 'src/api/department-and-division'
...
export default function SectionUpdateStatus() {
  ...
  const handleChangeStatus = async () => {
    ...
    runProcess('Wait for updating Department and Division')
    stopProcess()
    await changeStatusDepartmentAndDivision(dataForm)
      .then((res) => {
        if (res?.status === 200) {
  ...
  return (
    ...
    >
      ...
      <Typography.Title level={5} style={{ margin: 0 }}>
        {`Are you sure want to ${dataForm?.is_active ? 'activate' : 'inactivate'} this division & department ?`}
      </Typography.Title>
