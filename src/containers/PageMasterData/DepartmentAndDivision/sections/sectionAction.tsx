...
import { downloadTemplate, downloadData, uploadTemplate } from 'src/api/department-and-division'
...
export default function SectionAction() {
  ...
  async function downloadDataAction() {
    try {
      ...
      link.href = url
      link.setAttribute('download', 'division-and-department.xlsx')
  ...
  async function downloadTemplateAction() {
    try {
      ...
      const link = document.createElement('a')
      link.href = url
      link.setAttribute('download', 'template-division-and-department.xlsx')
      document.body.appendChild(link)
      link.click()
      document.body.removeChild(link)
    } catch (error) {
      ...
      if (errorStatus === 401) {
        message.error('Unauthorized. error permission. you don\'t have permission to access this resource');
  ...
  const moreContent = (
    <Row gutter={[10, 10]} style={{ fontWeight: 'bold', width: 200 }}>
      ...
      <Col span={24} style={{ cursor: 'pointer' }}>
        <Row gutter={10} style={{ alignItems: 'center' }}>
          <Col><ICHCFileXLSX /></Col>
          <Col
            onClick={() => downloadDataAction()}
            title={'Download Data'}
          >
            {'Download Data'}
          </Col>
          ...
          <Col md={16} sm={24} xs={24}>
            <Row align="middle">
              ...
              <Input
                required
                type="select"
                placeholder="Search Division or Departmen Name "
                value={search}
                onChange={(e: any) => {
