...
export const getListKPISetting = async (params: any): Promise<any> => {
  const response = await call({
    method: METHODS.GET,
    overrideBaseUrl: overrideBaseUrlHC,
    subUrl: '/v2/kpi-setting',
    data: {
      ...params,
      sort_by: '',
      sort_order: '',
...
export const getListKPISettingById = async (payload: any): Promise<any> => {
  const response = await call({
    method: METHODS.GET,
    overrideBaseUrl: overrideBaseUrlHC,
    subUrl: `/v2/kpi-setting/${payload?.id}`,
  })

  return response
}

export const createKPISetting = async (payload: any) => {
  const response = await call({
    method: METHODS.POST,
    subUrl: '/v2/kpi-setting',
    overrideBaseUrl: overrideBaseUrlHC,
...
export const updateKPISetting = async (payload: any) => {
  const response = await call({
    method: METHODS.PUT,
    overrideBaseUrl: overrideBaseUrlHC,
    subUrl: `/v2/kpi-setting/${payload?.id}`,
